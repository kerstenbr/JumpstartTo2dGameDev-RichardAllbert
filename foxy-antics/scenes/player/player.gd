...
# Called every frame. 'delta' is the elapsed time since the previous frame.$"../CanvasLayer"
...
func update_debug_label() -> void:
	debug_label.text = "floor: %s inv:%s\n%s\nvel: %0.f,%.0f\n%d" % [is_on_floor(), _invincible, PlayerState.keys()[_state] ,velocity.x, velocity.y, _lives]
