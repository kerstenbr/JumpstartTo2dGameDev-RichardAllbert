...
@onready var kicksound: AudioStreamPlayer2D = $Kicksound
...
const DRAG_LIM_MAX: Vector2 = Vector2(60, 60)
const DRAG_LIM_MIN: Vector2 = Vector2(-60, -60)
...
func _physics_process(delta: float) -> void:
	...
	label.text += "%.0f,%.0f" % [_dragged_vector.x, _dragged_vector.y]
...
func play_collision() -> void:
	if _last_collision_count == 0 and get_contact_count() > 0 and kicksound.playing == false:
		kicksound.play()
